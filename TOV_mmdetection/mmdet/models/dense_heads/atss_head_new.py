...
class ATSSHeadNew(AnchorHead):
    ...
    def __init__(self,
        ...
        self.dcn_cfg = dict(type='DCNv2', deform_groups=1, dilation=3)
        super(ATSSHeadNew, self).__init__(
            num_classes, in_channels, init_cfg=init_cfg, **kwargs)
        # self.involution = Involution(in_channels, kernel_size=5, stride=1, group_channels=64, reduction_ratio=16)
        self.involution = Involution(in_channels, kernel_size=7, stride=1, group_channels=16, reduction_ratio=4)
        self.downsample = nn.MaxPool2d(4)  # 添加一个降采样层
    ...
    def _init_layers(self):
        ...
        dcn_cfg = dict(type='DCNv2', deform_groups=1, dilation=3)
        ...
        dcn_cfg = dict(type='DCNv2', deform_groups=1, dilation=3)
        # 这里可以根据需要调整 DCN 的具体参数
        #
        # self.atss_cls = build_conv_layer(
        #     dcn_cfg,
        #     self.feat_channels,
        ...
        self.atss_reg = build_conv_layer(
            dcn_cfg,
            self.feat_channels,
            self.num_anchors * 4,
            3,
        ...
        self.atss_centerness = build_conv_layer(
            dcn_cfg,
            self.feat_channels,
            self.num_anchors * 1,
            3,
    ...
    def forward_single(self, x, scale):
        ...
        x_inv = self.involution(x_down)
        x_inv = x_down

        height, weight = cls_feat.shape[2], cls_feat.shape[3]
        x_inv_up = F.interpolate(x_inv, size=(height,weight), mode='bilinear')  # 上采样回原始尺寸
        # 添加残差连接
        cls_feat = cls_feat + x_inv_up
        reg_feat = reg_feat + x_inv_up
